...
if (req.http.Cookie ~ "cookies_policy") {
  if (req.http.Cookie:cookies_policy ~ "%22usage%22:true") {
    ...
    if (table.lookup(active_ab_tests, "TransitionUrgency3") == "true") {
      if (req.http.User-Agent ~ "^GOV\.UK Crawler Worker") {
        set req.http.GOVUK-ABTest-TransitionUrgency3 = "A";
      } else if (req.url ~ "[\?\&]ABTest-TransitionUrgency3=A(&|$)") {
        # Some users, such as remote testers, will be given a URL with a query string
        # to place them into a specific bucket.
        set req.http.GOVUK-ABTest-TransitionUrgency3 = "A";
      } else if (req.url ~ "[\?\&]ABTest-TransitionUrgency3=B(&|$)") {
        # Some users, such as remote testers, will be given a URL with a query string
        # to place them into a specific bucket.
        set req.http.GOVUK-ABTest-TransitionUrgency3 = "B";
      } else if (req.http.Cookie ~ "ABTest-TransitionUrgency3") {
        # Set the value of the header to whatever decision was previously made
        set req.http.GOVUK-ABTest-TransitionUrgency3 = req.http.Cookie:ABTest-TransitionUrgency3;
      } else {
        declare local var.denominator_TransitionUrgency3 INTEGER;
        declare local var.denominator_TransitionUrgency3_A INTEGER;
        declare local var.nominator_TransitionUrgency3_A INTEGER;
        set var.nominator_TransitionUrgency3_A = std.atoi(table.lookup(transitionurgency3_percentages, "A"));
        set var.denominator_TransitionUrgency3 += var.nominator_TransitionUrgency3_A;
        declare local var.denominator_TransitionUrgency3_B INTEGER;
        declare local var.nominator_TransitionUrgency3_B INTEGER;
        set var.nominator_TransitionUrgency3_B = std.atoi(table.lookup(transitionurgency3_percentages, "B"));
        set var.denominator_TransitionUrgency3 += var.nominator_TransitionUrgency3_B;
        set var.denominator_TransitionUrgency3_A = var.denominator_TransitionUrgency3;
        if (randombool(var.nominator_TransitionUrgency3_A, var.denominator_TransitionUrgency3_A)) {
          set req.http.GOVUK-ABTest-TransitionUrgency3 = "A";
        } else {
          set req.http.GOVUK-ABTest-TransitionUrgency3 = "B";
        }
...
sub vcl_deliver {
  ...
  if (req.http.Cookie ~ "cookies_policy") {
    if (req.http.Cookie:cookies_policy ~ "%22usage%22:true") {
      if (table.lookup(active_ab_tests, "TransitionUrgency3") == "true") {
        if (req.http.Cookie !~ "ABTest-TransitionUrgency3" || req.url ~ "[\?\&]ABTest-TransitionUrgency3" && req.http.User-Agent !~ "^GOV\.UK Crawler Worker") {
          set var.expiry = time.add(now, std.integer2time(std.atoi(table.lookup(ab_test_expiries, "TransitionUrgency3"))));
          add resp.http.Set-Cookie = "ABTest-TransitionUrgency3=" req.http.GOVUK-ABTest-TransitionUrgency3 "; secure; expires=" var.expiry "; path=/";
        }
      }
    }
  }
